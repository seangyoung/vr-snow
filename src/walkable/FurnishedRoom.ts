...
export interface RoomLayout {
  ...
  spawn: number[]; deskTarget: number[]; door: number[];
  furniture: Array<{ id: string; x: number; z: number; width: number; depth: number; height: number }>;
}
...
export class FurnishedRoom implements WalkableEnvironment {
  ...
  constructor(private readonly options: RoomOptions) {
    ...
    for (const f of layout.furniture) {
      ...
      if (f.id === options.interactionFurniture) desk=proxy;
    }
    if (!desk) throw new Error(`Missing interactive furniture: ${options.interactionFurniture}`);
    this.desk=desk;
  ...
  private async loadAssets(basePath: string): Promise<void> {
    try {
      const gltf=await new GLTFLoader().loadAsync(`${basePath}models/${this.options.asset}`);
      ...
      console.warn(`${this.options.name} model could not load; using the furnished fallback.`,error);
    }
  }
